...
class Pipeline {
 public:
  ...
  bool IsRayTracing() const { return pipeline_type_ == PipelineType::kRayTracing; }
  ...
  /// Returns success result if |shader| found and shader index in |out|.
  ...
  Result GetShaderIndex(Shader* shader, uint32_t* out) const {
    ...
    return Result("Referred in group shader not found");
  }
  ...
  /// The |sbt| must have a unique name over all SBTs in the script.
  ...
  /// The |group| must have a unique name over all shader groups in the
  /// script.
