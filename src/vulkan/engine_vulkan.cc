...
Result EngineVulkan::GetVkShaderStageInfo(
  ...
  VkShaderStageFlagBits stage = VK_SHADER_STAGE_FLAG_BITS_MAX_ENUM;
  Result r = ToVkShaderStage(shader_info.type, &stage);
  if (!r.IsSuccess())
    return r;
...
Result EngineVulkan::GetVkShaderStageInfo(
  ...
  if (pipeline->IsRayTracing()) {
    for (size_t i = 0; i < info.shader_info_rt.size(); i++) {
      Result r = GetVkShaderStageInfo(info.shader_info_rt[i], stage_info[i]);
      if (!r.IsSuccess())
        return r;
    }
  } else {
    ...
    for (auto& it : info.shader_info) {
      Result r = GetVkShaderStageInfo(it.second, stage_info[stage_count]);
      if (!r.IsSuccess())
        return r;
      if (it.first == VK_SHADER_STAGE_COMPUTE_BIT &&
